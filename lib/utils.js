...
module.exports.replicate = function replicate(argv) {
  ...
  prompt.get(schema, (err, {ident, secret}) => {
    prompt.stop()
    const {publicKey, secretKey} = HyperVault.passwdPair(ident, secret)
    ...
    vault.ready((err) => {
      ...
      // TODO: Trap SIGINT to do a clean-leave
    })
  })
...
module.exports.initRepo = function initRepo (argv) {
  ...
  function promptPasswdPair (callback) {
    ...
    prompt.get(schema, (err, {ident, secret, secret2}) => {
      prompt.stop()
      if (secret !== secret2) {
        process.stdout.write('Passwords mismatch!\n\n')
        process.exit(1)
      }
      callback(err, HyperVault.passwdPair(ident, secret))
    })
  }
}
