...
class MetaDrive extends hypercore {
  ...
  _initializeContent (callback) {
    ...
    // Use the binRef key to create a reciever feed
      .then(({action, header}) => {
  ...
  writeFile(name, data, opts, callback) {
    if(typeof opts === 'function') return this.writeFile(name, data, {}, opts)
    this.createWriteStream(name, opts, (err, stream) => {
      if (err) return callback(err)
      stream.end(Buffer.from(data), (binfeed) => {
        callback(null)
      })
    })
  }
