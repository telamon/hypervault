...
class MetaDrive extends hypercore {
  ...
  replicate (opts) {
    const stream = super.replicate(Object.assign(opts, {expectedFeeds: 2}))
    debug('replicating meta', this.key.toString('hex').substr(0,8))
    stream.on('finalize', () => {debug(this.key.toString('hex').substr(0,8), 'finalizing')})
    // Join the binfeed into repl stream once available
    this.binfeed((err, binfeed) => {
      debug(this.key.toString('hex').substr(0,8), 'joining binfeed into replication', binfeed.key.toString('hex').substr(0,8))
      if (err) stream.emit('error')
      binfeed.replicate({stream, expectedFeeds: 2})
    })
    // stream.on('feed', (key) => debug(this.key.toString('hex').substr(0,8), 'Feed recv:', key.toString('hex').substr(0,8)))
    return stream
  }
  ...
  binfeed (callback) {
    ...
    super.ready(() => {
      this._initializeContent(callback)
    })
  }
  ...
  _initializeContent (callback) {
    ...
    // happens only during replication and existing read-only feed load
      .then(({action, header}) => {
        ...
        debug(this.key.toString('hex').substr(0,8), 'allocating remote feed', header.bin.substr(0,8))
