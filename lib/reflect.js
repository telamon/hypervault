    ...
    fetchReflectCache(this._storage, (err, snapshot) => {
      ...
      _indexFolder(this.repo, (err, fsTree) => {
        ...
        const nextEntry = (i) => {
          ...
          const atime = a ? new Date(a.mtime) : undefined
          const btime = b ? new Date(b.mtime) : undefined
          const otime = o ? new Date(o.mtime) : undefined

          // ACTIONS
          const exp = () => {
            debug('EXPORT begin:', file)
            this.exportFile(file, path.join(this.repo, file), {wait: !!opts.wait}, err => {
              changeLog[file] = {action: 'export', err}
              debug('EXPORT done:', file)
              nextEntry(i + 1)
            })
          }

          const imp = () => {
            debug('IMPORT begin:', file)
            this.importFile(file, path.join(this.repo, file), err => {
              changeLog[file] = {action: 'import', err}
              debug('IMPORT done:', file)
              nextEntry(i + 1)
            })
          }

          const del = () => {
            if (a) fs.unlinkSync(path.join(this.repo, file)) // yeah uh, this is not callbackstyle
          ...
          // Unconditional export / unlesss it's a tombstone.
          if (b && !o && !a && !b.deleted) return exp()
          ...
          if (b && o && !a) {
            if (!b.deleted) {
              ...
              return nop('A lonely tombstone exists')
            }
          }

          // Perform 3-way merge
          if (b && o && a) {
            // Theoretically otime should equal Math.min(atime, btime)
            const aDiverged = atime > otime
            const bDiverged = btime > otime
            ...
            // No diversions
            if (!bDiverged && !aDiverged) return nop('already in sync')

            // Both diverged

            // B is newer
            if (btime >= atime) return b.deleted ? del() : exp()
            // A is newer
            if (btime < atime) return imp()
          }
