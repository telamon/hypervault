    ...
    fetchReflectCache(this._storage, (err, snapshot) => {
      ...
      _indexFolder(this.repo, (err, fsTree) => {
        ...
        const nextEntry = (i) => {
          ...
          // ACTIONS
          const exp = () => {
            this.exportFile(file, path.join(this.repo, file), err => {
              changeLog.push({file, action: 'exported', err})
              nextEntry(i + 1)
            })
          }
          ...
          const imp = () => {
            this.importFile(file, path.join(this.repo, file), err => {
              changeLog.push({file, action: 'imported', err})
              nextEntry(i + 1)
            })
          }
          ...
          const del = () => {
            return this.unlink(file, (err) => {
              changeLog.push({file, action: 'deleted', err})
              nextEntry(i + 1)
            })
          }

          const nop = (err) => {
            changeLog.push({file, action: 'ignored', err})
            nextEntry(i + 1)
          }
