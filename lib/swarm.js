...
class HypervaultSwarm {
  ...
  join (opts = {}) {
    ...
    this.net.discovery.holepunchable((err, yes) => {
      ...
      const discoveryKey = crypto
        .createHash('sha256')
        .update(this.vault.key)
        .digest()

      this.net.join(discoveryKey, {announce: true, lookup: true})
  ...
  onConnect (socket, details) {
    ...
    socket.once('error', (err) => {
      debug('REPL socket error, dropping peer', err)
      if (!socket.destroyed) socket.destroy()
      if (!coreStream.destroyed) coreStream.destroy()
    })
